...
class User
{
  ...
  static $USER_HOME = "";//"/home/kidthales/git/dev/viu_gcp/data/users/";
  ...
  static $DATABASE_LOCATION = "";//"/home/kidthales/git/dev/viu_gcp/data/users/";
  ...
  **/
  ...
  function __construct($config, $registration_callback=false)
  {
  	User::$USER_HOME = User::$DATABASE_LOCATION = $config['base_path'] . $config['user_db_path'];
  	User::$DATABASE_NAME = $config['user_db_name'];
  	User::$DOMAIN_NAME = $_SERVER['SERVER_NAME'];
  ...
  function rebuild_database($dbfile)
  {
    ...
    $this->database->commit();
		...
		function process(&$registration_callback=false)
		{
			...
			// HACK - Redirect to collections after login.
			if(isset($_POST["op"]) && $_POST["op"] == 'login' && $this->authenticated) {
				function urlFilter($var) {
					return !in_array($var, array('index.php', 'login'));
				}

				$url = "http" . (!empty($_SERVER['HTTPS'])?"s":"") . "://".$_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
				$url = array_filter(explode('/', $url), 'urlFilter');
				$url = preg_replace('/\?.*/', '', implode('/', $url));

				header('Location: ' . $url);
			}
			else if(isset($_POST["op"]) && $_POST["op"] == 'unregister' && !$this->authenticated) {
				$url = "http" . (!empty($_SERVER['HTTPS'])?"s":"") . "://".$_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
				$url = preg_replace('/login.*/', 'login', $url);
	...
	// ---------------------
		...
		function unregister()
		{
			...
			// viu_gcp modification.
			if(rmdir(User::$USER_HOME . $username)) {
			        global $accounts_available;
                                $accounts_available->replaceAccount();
                        }
